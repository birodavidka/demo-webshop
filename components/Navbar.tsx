...
import { Menu, X, ArrowRight, Zap, Search, ChevronDown } from "lucide-react";
import Link from "next/link";
import { navItems } from "@/app/shared/types"; // navItems-nek tartalmaznia kell hasDropdown + dropdownItems mezőket
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <nav className="hidden items-center space-x-1 lg:flex">
              {navItems.map((item) => {
                const showDropdown =
                  activeDropdown === item.name && item.dropdownItems?.length;
                return (
                  <motion.div
                    key={item.name}
                    variants={itemVariants}
                  ...
                  >
                    <Link
                      href={item.href}
                      className="text-foreground/80 hover:text-foreground relative rounded-lg px-4 py-2 text-sm font-medium transition-colors duration-200 flex items-center gap-1"
                    >
                    ...
                    <AnimatePresence>
                      {showDropdown && (
                        <motion.div
                          className="border-border bg-background/95 absolute left-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-xl border shadow-xl backdrop-blur-lg"
                ...
                <div className="space-y-1">
                  {navItems.map((item) => (
                    <motion.div key={item.name} variants={mobileItemVariants}>
                      ...
                      {/* Ha szeretnéd mobilon is mutatni a dropdown elemeket, itt felsorolhatod: */}
                        ...
                        <div className="ml-4">
                          {item.dropdownItems.map((dd) => (
                            <Link
                              key={dd.name}
                              href={dd.href}
                              className="text-muted-foreground hover:text-foreground block rounded-lg px-4 py-2 text-sm"
                              onClick={() => setIsMobileMenuOpen(false)}
                            >
                              {dd.name}
                            </Link>
